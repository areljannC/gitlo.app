...
describe('Card', () => {
	...
	it('should render a card with a name', async () => {
		const wrapper = await mountSuspended(Card, {
			global: { plugins: [pinia] },
			props: { cardId: MOCK_HASH[4] }
		});
	...
	it('should update the card name', async () => {
		const cardsStore = useCardsStore();
		const wrapper = await mountSuspended(Card, {
			global: { plugins: [pinia] },
			props: { cardId: MOCK_HASH[4] }
		...
		expect(cardsStore.getCardById(MOCK_HASH[4])?.name).toBe('Updated Card Name');
	});

	it('should update the card name when `Enter` is pressed', async () => {
		const cardsStore = useCardsStore();
		const wrapper = await mountSuspended(Card, {
			global: { plugins: [pinia] },
			props: { cardId: MOCK_HASH[4] }
